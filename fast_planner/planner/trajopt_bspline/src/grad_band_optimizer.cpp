...
void GradBandOptimizer::setParameterAuto(double max_v, double max_a) {
  ...
  // lamda1 = 1.0;    // Curvature
  // lamda2 = 1.0;    // Distance (先设小点看通不通)
  // lamda3 = 1.0;    // Feasibility (调大它！)
  // lamda4 = 0.01;
  // dist0  = 0.5;    // 安全半径
  // max_iteration_num = 200; // 迭代次数多给点
  // algorithm = 11;  // L-BFGS
...
void GradBandOptimizer::optimizeTrajFixedEnd() {
  ...
  // std::cout << "endid =" << end_id << " start_id" << start_id << std::endl;
  ...
  try {
    ...
    vec_time.clear();
    vec_cost.clear();
    time_start =  quadrotor_msgs::Time::now();//ros::Time::now();
    nlopt::result result = opt.optimize(q, final_cost);

    //STCY << "Min cost:" << min_cost << REC;
...
double GradBandOptimizer::costFuncMinCurvaturePara(const std::vector<double>& x,
  ...
  for (int i = 0; i < opt->control_points.rows(); ++i) {
    ...
    if (dist < opt->dist0)
    {
      // std::cout << "getDistanceAndGradient dist =" << dist << "grad ="  << dist_grad << std::endl;
      f2 = pow(dist - opt->dist0, 2);
    }
    f_dist += f2;
  ...
  /* evaluation */  //jcy to do
  quadrotor_msgs::Time te1 = quadrotor_msgs::Time::now();//ros::Time::now();
...
double GradBandOptimizer::costFuncMinJerkPara(const std::vector<double>& x,
  ...
  for (int i = 0; i < opt->control_points.rows(); ++i) {
    ...
    if(dist < opt->dist0){
    // std::cout << " dist =" << dist <<  " opt->dist0 =" << opt->dist0 << " f_dist =" << f_dist << "f2 =" << f2 << std::endl;
    // std::cout << "q2 =" << q2 << std::endl;
    }
    /* only constrain the first few points */
    static double vm = opt->max_vel, am = opt->max_acc,
    ...
    for (int j = 0; j < 3; ++j) {
      cona132[j] = q132(j) * q132(j) * ts_inv * ts_inv * ts_inv * ts_inv - am2;
      //  std::cout << "q132(j) =" << q132(j) << "j =" << j << std::endl;
      // std::cout << "cona132[j] =" << cona132[j] << std::endl;
      if (cona132[j] > 0.0) fa += pow(cona132[j], 2);
    ...
    // std::cout << "ts_inv  =" << ts_inv  << std::endl;
  ...
  std::cout << " f_curvature =" << f_curvature << "f_exp_vel + f_exp_acc = " << f_exp_vel + f_exp_acc<<std::endl;
  std::cout << " f_dist =" << f_dist << std::endl;
  std::cout << " opt->lamda1 =" <<  opt->lamda1 << std::endl;
  std::cout << " opt->lamda3 =" <<  opt->lamda3  << std::endl;
  // if (optnum % 40 == 0) {
  ...
  quadrotor_msgs::Time te1 = quadrotor_msgs::Time::now();
  double time_now = (te1 - opt->time_start).toSec();
  opt->vec_time.push_back(time_now);
  if (opt->vec_cost.size() == 0) {
    opt->vec_cost.push_back(f_combine);
